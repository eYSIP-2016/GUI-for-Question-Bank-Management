        ...
        /*************Storing equations******************/
 		...
 		if (!is_null($exp)&&!empty($exp)) {
			$equation = new equations();
	 		...
	 		$equation_URL = "https://latex.codecogs.com/gif.latex?int%20x=a+b;" ;//Request::get('hidden_exp_url'); 
	 	...
	 	if(!is_null(Request::file('Q_diagram'))){
	            ...
	            $path = URL::to('/');
	            $question->diagram_path = $path.'/images/'.$name;
 	...
 	public function getSearchResults(){
 		...
 		if(empty($search_string)){
            ...
            $questions = DB::table('q_tables')
            			...
            			->whereIn('q_tables.q_id',$question_from_tags)->orderBy('q_tables.updated_at','asc');
        }
        else{
        	$tags =  tags::lists('name','id');
            $questions = DB::table('q_tables')
            			...
            			->orderBy('q_tables.updated_at','asc');
        }
